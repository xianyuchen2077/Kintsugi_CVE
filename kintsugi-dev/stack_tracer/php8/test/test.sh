...
# PHP Tracer automated test script
# Compiles, deploys, and tests using sysdig to monitor syscalls
...
TEST_CONTAINER_NAME="stack-tracer-php8-test"
BUILD_SCRIPT="$SOURCE_ROOT/source/build.sh"
LOG_DIR="$SCRIPT_DIR/log"

echo "======================================================"
echo "    PHP8 Tracer Automated Test"
echo "======================================================"
...
for i in {1..11}; do
    if [ -f "$SCRIPT_DIR/web/${i}.php" ]; then
        ...
        echo "  Starting sysdig monitor..."
        # -s 8192: increase capture buffer length (default 80 bytes)
        # -v: verbose mode, show full buffer content
        ...
        sleep 5
        echo "  Monitor ready, executing PHP"
        ...
        sleep 5
        echo "  Waiting for syscalls to complete"

        echo "  Stopping sysdig monitor..."
        kill $SYSDIG_PID
