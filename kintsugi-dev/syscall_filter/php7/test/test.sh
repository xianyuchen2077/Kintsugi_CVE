...
TEST_CONTAINER_NAME="syscall-filter-php7-test"
BUILD_SCRIPT="$SOURCE_ROOT/source/build.sh"

echo "======================================================"
echo "    PHP7 syscall_filter 自动测试脚本"
echo "======================================================"
...
# 测试4: large_ruleset_test.php (MAX_RULES=128 扩容测试)
echo ""
echo "--- 测试4: large_ruleset_test.php (MAX_RULES=128 扩容测试) ---"
echo "测试大规则集: 40 slots (8 basic + 16 open paths + 16 openat paths)"
if docker exec "$CONTAINER_ID" php /test/large_ruleset_test.php; then
    echo ""
    echo "✓ large_ruleset_test.php 执行成功 - MAX_RULES=128 扩容验证通过"
else
    echo ""
    echo "✗ large_ruleset_test.php 执行失败"
    exit 1
fi
