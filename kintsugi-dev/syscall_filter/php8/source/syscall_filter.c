...
static uint32_t hash_string(const char *str, uint8_t len)
{
    ...
    for (uint8_t i = 0; i < len && i < MAX_PATH_LEN && str[i] != '\0'; i++) {
        hash ^= (uint32_t)str[i];
...
static void syscall_filter_execute_begin(INTERNAL_FUNCTION_PARAMETERS)
{
    ...
    HashTable *ht = Z_ARRVAL_P(syscalls_arg);
    zval *entry;
    zend_string *key;
    zend_ulong idx;

    uint32_t rule_idx = 0;  // 全局规则计数器

    // 遍历PHP数组,展开到三个并行数组
    ZEND_HASH_FOREACH_KEY_VAL(ht, idx, key, entry) {
        // 必须是关联数组（key必须是字符串）
        if (!key) {
            php_error_docref(NULL, E_WARNING, "Must use associative array: 'syscall' => [paths]");
            continue;
        }

        const char *syscall_name = ZSTR_VAL(key);
        int syscall_nr = get_syscall_nr(syscall_name);

        if (syscall_nr < 0) {
            php_error_docref(NULL, E_WARNING, "Unknown syscall: %s", syscall_name);
            continue;
        }

        // 值必须是数组
        if (Z_TYPE_P(entry) != IS_ARRAY) {
            php_error_docref(NULL, E_WARNING, "Value for '%s' must be an array (use empty array [] for no path constraint)", syscall_name);
            continue;
        }

        HashTable *path_ht = Z_ARRVAL_P(entry);
        uint32_t path_count = zend_hash_num_elements(path_ht);

        if (path_count == 0) {
            // 情况A: 无路径约束 (空数组 [])
            if (rule_idx >= MAX_RULES) {
                php_error_docref(NULL, E_ERROR, "Exceeded max rules limit (%d)", MAX_RULES);
                RETURN_FALSE;
            }
            ...
            // 情况B: 有路径约束 - 每个路径占一个槽位
            zval *path_entry;
            ZEND_HASH_FOREACH_VAL(path_ht, path_entry) {
                if (rule_idx >= MAX_RULES) {
                    php_error_docref(NULL, E_ERROR, "Exceeded max rules limit (%d)", MAX_RULES);
                    RETURN_FALSE;
                }
                ...
                if (Z_TYPE_P(path_entry) != IS_STRING) {
                    php_error_docref(NULL, E_WARNING, "Path must be string for %s", syscall_name);
                    continue;
                }

                const char *path = Z_STRVAL_P(path_entry);
                size_t path_len = Z_STRLEN_P(path_entry);

                if (path_len >= MAX_PATH_LEN) {
                    php_error_docref(NULL, E_WARNING, "Path too long (max %d): %s", MAX_PATH_LEN-1, path);
                    continue;
                }
    ...
    if (send_packet(&pkt) != 0) {
        php_error_docref(NULL, E_WARNING, "Failed to send CMD_FILTER_BEGIN");
        RETURN_FALSE;
    }
...
// syscall_filter_end: 批量删除规则并停止监控
static void syscall_filter_execute_end(INTERNAL_FUNCTION_PARAMETERS)
{
    ...
    if (send_packet(&pkt) != 0) {
        php_error_docref(NULL, E_WARNING, "Failed to send CMD_FILTER_END");
        RETURN_FALSE;
    }
...
PHP_MINFO_FUNCTION(syscall_filter)
{
    ...
    php_info_print_table_row(2, "Max syscalls per command", "64");
    php_info_print_table_row(2, "Max paths per syscall", "16");
