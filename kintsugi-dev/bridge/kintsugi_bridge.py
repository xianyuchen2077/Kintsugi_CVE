...
class RunRequest(BaseModel):
    ...
    feedback_attempts: int = Field(1, ge=0, le=10)
...
def _merge_feedback_stage_timing(timing: dict, feedback: dict | list | None) -> dict:
    """Expose feedback-loop stage durations as normal stage_* keys for the UI track."""
    merged = dict(timing)
    ...
    for attempt in attempts:
        ...
        for key, value in stage_durations.items():
            if key.startswith("stage_") and isinstance(value, (int, float)):
...
def scan_artifact_runs(cve: str | None = None) -> list[dict]:
    ...
    for language_dir in sorted(path for path in data_root.iterdir() if path.is_dir() and not path.name.startswith(".")):
        ...
        for data_dir in sorted(path for path in language_dir.iterdir() if path.is_dir()):
            ...
            if active_job:
                ...
                if active_job.artifact_directory:
                    display_directory = active_job.artifact_directory
                    item_id = f"ART-{language}-{Path(display_directory).name}"
            stage_count = sum(1 for key in timing_for_ui if key.startswith("stage_"))
            items.append({
                "id": item_id,
                "cve": cve_id,
                "language": language,
    ...
    for language, job in active_jobs:
        ...
        directory = job.artifact_directory or f"data/{language}/{job.request.cve}"
        items.append({
...
def _stage_statuses_for_artifact(data_dir: Path, timing: dict, feedback: dict) -> list[dict]:
    statuses: list[dict] = []
    feedback_stages: set[int] = set()
    if isinstance(feedback, dict):
        for attempt in feedback.get("attempts") or []:
            if not isinstance(attempt, dict):
                continue
            if not attempt.get("feedback_triggered"):
                continue
            route_stage = attempt.get("route_stage")
            if isinstance(route_stage, int):
                feedback_stages.add(route_stage)
            stage_durations = attempt.get("stage_durations")
            if isinstance(stage_durations, dict):
                for key in stage_durations:
                    if key.startswith("stage_"):
                        try:
                            feedback_stages.add(int(key.split("_", 1)[1]))
                        except ValueError:
                            pass

    repair_stats = _load_json(data_dir / "repair" / "repair_stats.json")
    repair_totals = repair_stats.get("totals", {}) if isinstance(repair_stats, dict) else {}

    checks = {
        0: (True, "timing.json"),
        1: (True, "timing.json"),
        2: (
            (data_dir / "normal.scap").exists() or (data_dir / "malicious.scap").exists(),
            "normal.scap / malicious.scap",
        ),
        3: (
            (data_dir / "normal.jsonl").exists() or (data_dir / "malicious.jsonl").exists(),
            "normal.jsonl / malicious.jsonl",
        ),
        4: (_has_files(data_dir / "unit", "*.csv"), "unit/*.csv"),
        5: (_has_files(data_dir / "callstack", "*.jsonl"), "callstack/*.jsonl"),
        6: (_has_files(data_dir / "callstack_with_code", "*.json"), "callstack_with_code/*.json"),
        7: (_has_files(data_dir / "detect", "*_pairs.json"), "detect/*_pairs.json"),
        8: (_has_files(data_dir / "repair", "*_repairs.json"), "repair/*_repairs.json"),
        9: (_has_files(data_dir / "repair_with_whitelist", "*_repairs.json"), "repair_with_whitelist/*_repairs.json"),
        10: (_has_files(data_dir / "repair_validated", "*_repairs.json"), "repair_validated/*_repairs.json"),
        11: (True, "timing.json"),
    }

    for stage in range(12):
        key = f"stage_{stage}"
        timing_item = timing.get(key)
        duration = timing_item.get("duration") if isinstance(timing_item, dict) else None
        has_timing = isinstance(timing_item, dict)
        evidence_ok, evidence_source = checks[stage]
        status = "missing"
        source = "未找到运行证据"
        if has_timing and evidence_ok:
            status = "feedback" if stage in feedback_stages else "done"
            source = "反馈循环" if stage in feedback_stages else evidence_source
        elif has_timing:
            status = "failed"
            source = f"缺少预期产物：{evidence_source}"
            if stage == 8 and isinstance(repair_totals, dict):
                generated = repair_totals.get("repairs_generated")
                below = repair_totals.get("skipped_below_threshold")
                source = f"Stage 8 未生成修复；repairs_generated={generated}, skipped_below_threshold={below}"
        statuses.append({"stage": stage, "status": status, "duration": duration, "source": source})
    return statuses
...
def artifact_detail(directory: str) -> dict:
    ...
    return {
        ...
        "timing": timing if isinstance(timing, dict) else {},
        "stage_statuses": _stage_statuses_for_artifact(data_dir, timing if isinstance(timing, dict) else {}, feedback_dict),
...
def update_job_stage_from_log(job: Job, line: str) -> None:
    ...
    lowered = line.lower()
    if "return to stage" in lowered:
        job.current_stage_label = f"Feedback route to Stage {stage}"
    elif "executing from stage" in lowered:
        job.current_stage_label = f"Stage {stage}"
